...
class CalificacionServicios extends MainService
{
    ...
    {
        $stmt = $this->conexion->prepare("UPDATE alumno_asignatura_periodo SET NOTA4=?,NOTA5=?,NOTA6=?
    ...
    {
        $datos = $this->conexion->query("SELECT PROMEDIOQ1 FROM matricula_periodo WHERE COD_ALUMNO='".$cod_alumno."'
        ...
        $stmt = $this->conexion->prepare("UPDATE matricula_periodo SET PROMEDIOQ1=?
    ...
    {
        $datos = $this->conexion->query("SELECT PROMEDIOQ2 FROM matricula_periodo WHERE COD_ALUMNO='".$cod_alumno."'
                                         AND COD_PERIODO_LECTIVO='".$cod_periodo."' AND COD_NIVEL_EDUCATIVO='".$cod_nivel_educativo."'");
        $promedio = $datos->fetch_assoc();
        ...
        $calificacionQ1 = $this->conexion->query("SELECT PROMEDIOQ1 FROM matricula_periodo WHERE COD_ALUMNO='".$cod_alumno."'
                                                  AND COD_PERIODO_LECTIVO='".$cod_periodo."' AND COD_NIVEL_EDUCATIVO='".$cod_nivel_educativo."'");
        $promedioQ1 = $calificacionQ1->fetch_assoc();
        $promedioFinal = (floatval($promedioQ1['PROMEDIOQ1']) + $promedioT)/2;
        

        $stmt = $this->conexion->prepare("UPDATE matricula_periodo SET PROMEDIOQ2=?, PROMEDIO_FINAL=?
                                          WHERE COD_ALUMNO=? AND COD_PERIODO_LECTIVO=? AND COD_NIVEL_EDUCATIVO=?");
    ...
    {
        return $this->conexion->query("SELECT NOTA1,NOTA2,NOTA3 FROM alumno_asignatura_periodo WHERE COD_ALUMNO='".$cod_alumno."'
        AND COD_ASIGNATURA='".$cod_asignatura."'");
    }
    function calificacionesEstudiante2($cod_alumno,$cod_asignatura)
    {
        return $this->conexion->query("SELECT NOTA4,NOTA5,NOTA6 FROM alumno_asignatura_periodo WHERE COD_ALUMNO='".$cod_alumno."'
        AND COD_ASIGNATURA='".$cod_asignatura."'");
    }
    function calificacionGeneral1($cod_alumno,$cod_periodo)
    {
        return $this->conexion->query("SELECT alumno_asignatura_periodo.NOTA1, alumno_asignatura_periodo.NOTA2, alumno_asignatura_periodo.NOTA3,asignatura.NOMBRE
    ...
    {
        return $this->conexion->query("SELECT alumno_asignatura_periodo.NOTA4, alumno_asignatura_periodo.NOTA5, alumno_asignatura_periodo.NOTA6,asignatura.NOMBRE
        FROM alumno_asignatura_periodo
        INNER JOIN asignatura ON asignatura.COD_ASIGNATURA = alumno_asignatura_periodo.COD_ASIGNATURA
        WHERE alumno_asignatura_periodo.COD_ALUMNO='".$cod_alumno."' AND alumno_asignatura_periodo.COD_PERIODO_LECTIVO='".$cod_periodo."'");
    }
